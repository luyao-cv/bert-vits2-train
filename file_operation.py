...
def download_model_from_oss(
    ...
    if osp.exists(model_basename) and osp.isdir(model_basename):
        ...
        print("download_model_from_oss cur_model_workspace: ", cur_model_workspace)
        ...
        if not osp.exists(local_model_dir) or not osp.exists(local_model_file):
            ...
            if not osp.exists(local_model_file):
                ...
                if exist:
                    oss_bucket.get_object_to_file(bucket_file, local_model_file)
                else:
...
def download_and_unzip_resousrce(model, model_revision=None):
    if osp.exists(model):
        ...
        check_local_model_is_latest(
            model_cache_dir, user_agent={ThirdParty.KEY: "speech_tts_autolabel"}
        )
    else:
...
def download_model(model, model_revision=None):
    if osp.exists(model):
        ...
        with zipfile.ZipFile(
            os.path.join(model_cache_dir, "resource.zip"), "r"
        ) as zipf:
...
def zip_and_upload_to_oss(session, source, oss_bucket, bucket_user_models_folder):
    pardir = osp.abspath(osp.dirname(source))
    dir_basename = osp.basename(source)
    target_zip_path = osp.join(pardir, dir_basename + ".zip")

    z = zipfile.ZipFile(
        file=osp.abspath(target_zip_path), mode="w", compression=zipfile.ZIP_DEFLATED
    )
    ...
    user_id = session["modelscope_uuid"]
    bucket_cur_folder = (
        bucket_user_models_folder + user_id + "/models/" + dir_basename + ".zip"
    )
...
def get_all_model_name_from_oss(session, oss_bucket, bucket_user_models_folder):
