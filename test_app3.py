...
def launch_infer_task(user_models, text, session):
    ...
    if  session['model_list'][user_models] != "凝光_ZH":
        ...
        # import pdb
        # pdb.set_trace()
        if default_model != os.path.join(inference_datasets_dir, f"{session['model_list'][user_models]}.pth"):
            default_model = os.path.join(inference_datasets_dir, f"{session['model_list'][user_models]}.pth")
            net_g = get_net_g(
        ...
        if default_model != f"Data/inference/G_0.pth":
            default_model = os.path.join(inference_datasets_dir, f"Data/inference/G_0.pth")
            net_g = get_net_g(
                model_path=config.webui_config.model, version=version, device=device, hps=hps
            )
            NAIVE_LOG('切换模型：', session['model_list'][user_models])
...
def launch_training_task(user_name, session, *audio_lst):
    ...
    os.system("python resample.py --in_dir {} --out_dir {}".format(cur_data_path, cur_data_path))
    yield ["数据处理完成，请继续等待......", None, session]
    yield ["采样 resample.py --in_dir {} --out_dir {}".format(cur_data_path, cur_data_path), None, session]
    ...
    shutil.copy(osp.join(inference_datasets_dir, "D_0.pth"), osp.join(train_datasets_dir, "D_0.pth"))
    shutil.copy(osp.join(inference_datasets_dir, "G_0.pth"), osp.join(train_datasets_dir, "G_0.pth"))
    shutil.copy(osp.join(inference_datasets_dir, "DUR_0.pth"),osp.join(train_datasets_dir, "DUR_0.pth"))
    ...
    shutil.move(osp.join(train_datasets_dir, sorted_g_files[-1]),
    ...
    yield ["训练完成，当前已等待{}秒...请点击模型体验来体验合成的效果吧......".format(passed_time), gr.Radio.update(choices=HOT_MODELS + session['my_models'], value="\N{bust in silhouette}" + user_name), session]
    return ["训练完成，当前已等待{}秒...请点击模型体验来体验合成的效果吧......".format(passed_time), gr.Radio.update(choices=HOT_MODELS + session['my_models'], value="\N{bust in silhouette}" + user_name), session]
...
with gr.Blocks(css="#warning {color: red} .feedback {font-size: 24px}") as demo:
    ...
    with gr.Tabs():
        with gr.TabItem("\N{rocket}模型定制") as tab_train:
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    audio_lst1 = [
                        gr.Audio(source="microphone", label="1. 清晨，阳光洒满小路，花儿含笑迎接新一天。"),
                        gr.Audio(source="microphone", label="2. 树梢上鸟儿歌唱，旋律悠扬。公园中，松鼠忙碌，秋日收获。"),
                        gr.Audio(source="microphone", label="3. 花园里，蝴蝶轻舞，狗狗追逐着飘落的叶子，欢乐满溢。"),
                        gr.Audio(source="microphone", label="4. 河边，小鸭戏水，水波荡漾，映出欢快身影。"),
                        gr.Audio(source="microphone", label="5. 蓝天下，花儿开放，绽放生机，小猫追逐彩色的梦。"),
                    ]
                with gr.Column(scale=1):
                    audio_lst2 = [
                        gr.Audio(source="microphone", label="6. 林间小径，草地上，松鼠跳跃，觅得果实分享喜悦。"),
                        gr.Audio(source="microphone", label="7. 海边，螃蟹踏着轻快步伐，留下一串足迹。"),
                        gr.Audio(source="microphone", label="8. 夕阳西下，蝴蝶飞舞，小兔听见晚风诉说着故事。"),
                        gr.Audio(source="microphone", label="9. 星空闪烁，小狐狸仰望，许下心中愿望。"),
                        gr.Audio(source="microphone", label="10. 早市上，小猪噜噜噜，探寻新鲜蔬果的香气。"),
                    ]
                with gr.Column(scale=1):
                    audio_lst3 = [
                        gr.Audio(source="microphone", label="11. Dawn breaks, and the path is bathed in sunlight, flowers smiling at the day."),
                        gr.Audio(source="microphone", label="12. By the river, ducklings splash, rippling water reflects their joy."),
                        gr.Audio(source="microphone", label="13. On the hill, sheep graze leisurely, a cool breeze whispers."),
                        gr.Audio(source="microphone", label="14. Under the blue sky, a rainbow stretches, a kitten chases colorful dreams."),
                        gr.Audio(source="microphone", label="15. In the woods, squirrels leap, sharing the joy of found fruits."),
                    ]
                with gr.Column(scale=1):
                    audio_lst4 = [
                        gr.Audio(source="microphone", label="16. At the beach, crabs walk briskly, leaving a trail of footprints."),
                        gr.Audio(source="microphone", label="17. As the sun sets, a bunny hears stories whispered by the evening wind."),
                        gr.Audio(source="microphone", label="18. Stars twinkle, a fox gazes up, making a secret wish."),
                        gr.Audio(source="microphone", label="19. In the morning market, a piglet sniffs, seeking the scent of fresh produce."),
                        gr.Audio(source="microphone", label="20. On the meadow, butterflies flutter, flowers bloom, bursting with life."),
                    ]
            # One task at a time
        ...
        with gr.TabItem("\N{party popper}模型体验") as tab_infer:
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    text_input = gr.Textbox(label="合成内容", value="清晨，阳光洒满小路，花儿含笑迎接新一天。", max_lines=3)
...
# demo.queue(concurrency_count=2).launch(server_name="10.21.226.169",server_port=8912,share=True)
demo.queue(concurrency_count=511, max_size=1022).launch(share=True, server_name="10.21.226.169", server_port=8912, ssl_certfile="cert.pem", ssl_keyfile="key.pem", ssl_verify=False)
