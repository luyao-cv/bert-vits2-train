...
from file_operation import (
    download_and_unzip_resousrce,
    download_model,
    download_model_from_oss,
    get_all_model_name_from_oss,
    oss_auth_check,
...
AUTO_LABEL_RESOURCE_DIR = download_and_unzip_resousrce(
    DEFAULT_RESOURCE_MODEL_ID, "v1.0.7"
)
...
def copy_importance_for_infer(src: str, dest: str):
    ...
    shutil.copy(
        osp.join(src, "tmp_am", "ckpt", lastest_ckpt_fn),
        osp.join(dest, "tmp_am", "ckpt", lastest_ckpt_fn),
    )
    shutil.copy(
        osp.join(src, "tmp_am", "config.yaml"), osp.join(dest, "tmp_am", "config.yaml")
    )

    os.makedirs(osp.join(dest, "data"), exist_ok=True)
    os.makedirs(osp.join(dest, "data", "se"), exist_ok=True)
    shutil.copy(
        osp.join(src, "data", "se", "se.npy"), osp.join(dest, "data", "se", "se.npy")
    )
...
def infer_task(text, session):
    ...
    try:
        ...
        session = get_all_model_name_from_oss(
            session, oss_bucket, bucket_user_models_folder
        )
        model_dir, local_flag = download_model_from_oss(
            session,
            oss_bucket,
            ROOT_WORKSPACE,
        ...
        command = "python infer_script.py --text '{}' --model_dir {} --BASE_MODEL_DIR {} --local_flag {} --output_file {} --user_id {}".format(
            text, model_dir, BASE_MODEL_DIR, local_flag, output_file, user_id
        )
        print(command)
        os.system(command)
...
def launch_infer_task(uuid_txt, user_model, text, session):
    ...
    before_queue_size = len(inference_not_done) - sum(
        1 for f in inference_not_done if f.done()
    )
    ...
    while not future.done():
        ...
        if not is_processing:
            ...
            yield [
                "排队等待资源中，前方还有{}个合成任务, 预计需要等待{}分钟...".format(
                    to_wait, round(to_wait * 0.3, 2)
                ),
                None,
            ]
        else:
...
def training_task(name, cur_data_path, cur_model_path, session):
    ...
    user_id = session.get("modelscope_uuid", "guest")
    try:
        command = "python train_script.py --data_path {} --model_path {} --user_id {} --AUTO_LABEL_RESOURCE_DIR {}".format(
            cur_data_path, cur_model_path, user_id, AUTO_LABEL_RESOURCE_DIR
        )
        print(command)
        ...
        zip_and_upload_to_oss(
            session, cur_model_path, oss_bucket, bucket_user_models_folder
        )
...
def launch_training_task(uuid_txt, session, *audio_lst):
    ...
    os.makedirs(cur_data_path, exist_ok=True)  # <workspace>/<uuid>/data/202303142248
    os.makedirs(user_models, exist_ok=True)  # <workspace>/<uuid>/models
    os.makedirs(cur_model_path, exist_ok=True)  # <workspace>/<uuid>/models/202303142248
    ...
    before_queue_size = len(training_not_done) - sum(
        1 for f in training_not_done if f.done()
    )
    before_done_count = training_done_count

    NAIVE_LOG(
        "Launching training task..., not done tasks: {}".format(before_queue_size),
        user_id,
    )
    future = training_threadpool.submit(
        training_task, name, cur_data_path, cur_model_path, session
    )
    ...
    while not future.done():
        ...
        if not is_processing:
            ...
            start_time = time.time()
            yield [
                "排队等待资源中，您前面还有{}个用户, 预计需要等待{}分钟...".format(
                    to_wait, to_wait * 6
                ),
                None,
                session,
            ...
            yield [
                "训练中, 预计需要600秒, 请耐心等待, 当前已等待{}秒...".format(
                    passed_time
                ),
                None,
                session,
    ...
    if success:
        ...
        yield [
            ...
            gr.Radio.update(
                ...
                value="\N{bust in silhouette}" + name,
            ),
            session,
        ]
        return [
...
with gr.Blocks(css="#warning {color: red} .feedback {font-size: 24px}") as demo:
        ...
        with gr.TabItem("\N{rocket}模型定制") as tab_train:
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    audio_lst1 = [
                        gr.Audio(
                            source="microphone", label="1. 希望我们大家都能像他一样"
                        ),
                        gr.Audio(
                            source="microphone",
                            label="2. 不行, 他想了一下, 我不能这样对国王说, 这是在撒谎",
                        ),
                        gr.Audio(
                            source="microphone",
                            label="3. 但他们非常和气地问她说, 你叫什么名字",
                        ),
                        gr.Audio(
                            source="microphone",
                            label="4. 鸭子心想, 我必须去拿回我的软糖豆",
                        ),
                        gr.Audio(
                            source="microphone", label="5. 小朋友, 你们不要再欺负它了"
                        ),
                    ]
                with gr.Column(scale=1):
                    audio_lst2 = [
                        gr.Audio(
                            source="microphone", label="6. 可是, 小黄鸭并不怕他们"
                        ),
                        gr.Audio(
                            source="microphone",
                            label="7. 然后, 他们一起走了很长一段时间",
                        ),
                        gr.Audio(
                            source="microphone", label="8. 突然, 墙壁后面传来一阵声音"
                        ),
                        gr.Audio(
                            source="microphone", label="9. 结果盘子掉在地上, 打得粉碎"
                        ),
                        gr.Audio(
                            source="microphone",
                            label="10. 四个小伙伴很开心, 一起感谢小松鼠的帮助",
                        ),
                    ]
                with gr.Column(scale=1):
                    audio_lst3 = [
                        gr.Audio(
                            source="microphone",
                            label="11. 不过, 当他看到拇指姑娘的时候, 他马上就变得高兴起来",
                        ),
                        gr.Audio(
                            source="microphone",
                            label="12. 从此以后, 他过上了幸福的生活",
                        ),
                        gr.Audio(
                            source="microphone",
                            label="13. 老山羊最后伤心地, 哭着走了出去",
                        ),
                        gr.Audio(
                            source="microphone",
                            label="14. 而且准备一直找下去, 直到他走不动为止",
                        ),
                ...
                with gr.Column(scale=1):
                    audio_lst4 = [
                        gr.Audio(
                            source="microphone",
                            label="16. 一起高高兴兴地, 回到了他们的爸爸妈妈身边",
                        ),
                        gr.Audio(
                            source="microphone",
                            label="17. 艾丽莎很小不能去上学, 但她有一个非常贵重精美的画册",
                        ),
                        gr.Audio(
                            source="microphone",
                            label="18. 狮子还是够不着, 它叫来了狐狸",
                        ),
                        gr.Audio(
                            source="microphone",
                            label="19. 姑娘坐到国王的马车上, 和国王一起回到宫中",
                        ),
                        gr.Audio(
                            source="microphone",
                            label="20. 温妮大叫了起来, 现在我们该怎么回家呀",
                        ),
                    ]
            # One task at a time
    ...
    refresh_button.click(
        fetch_uuid,
        inputs=[uuid_txt, session],
        outputs=[dbg_output, session, user_models],
    )
